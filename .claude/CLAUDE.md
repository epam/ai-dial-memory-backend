...
Quick Reference (do NOT skip steps)
    ...
    Epic — bd create -t epic "Goal" (container for intent + context)
    Brainstorm — superpowers:brainstorming (design before code)
    Plan — superpowers:writing-plans (2-5 min tasks)
    Sub-tasks — bd create for each + bd dep add (parent-child, blocks)
    Isolate — superpowers:using-git-worktrees (non-trivial work)
    Implement — bd ready → pick → bd update --claim → TDD (RED → GREEN → REFACTOR)
    Review — superpowers:requesting-code-review
    Verify — superpowers:verification-before-completion (evidence before claims)
    Finish — superpowers:finishing-a-development-branch
    Close — bd close <epic-id> --reason "Done"
