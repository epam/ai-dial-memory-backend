...
async def test_store_appends_row_and_returns_stored_true() -> None:
    ...
    result = await svc.store(
        "key",
        StoreMemoryInput(
            content="hello", memory_type="core", context="ctx", importance=0.8
        ),
    )
...
async def test_retrieve_with_app_name_returns_core_and_episodic() -> None:
    ...
    repo.filter_by_context.return_value = [
        _make_retrieve_row("e1", "episodic", context="my-app")
    ]
    ...
    assert "c1" in ids
    assert "e1" in ids
    repo.filter_by_context.assert_called_once_with(
        "bucket-id", "my-app", "episodic", 10
    )
